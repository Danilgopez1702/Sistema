...
if ($_SESSION['rol'] == 3132 || $_SESSION['rol'] == 1) {
    ...
    if (mysqli_num_rows($consultar_factura) < 0) {
        ...
        if ($status_factura == 1) {
            ...
            $mensajes = 'El usuario: ' . $nombre_usuario . ' Habilito la facturacion del cliente: ' . $numero_cliente;
            ...
            $mensajes = 'El usuario: ' . $nombre_usuario . ' Deshabilito la facturacion del cliente: ' . $numero_cliente;
            $log = mysqli_query($conexion, "INSERT INTO `log`(`accion_log`, `id_usuario`, `id_cliente`) VALUES ('$mensajes,'$id_usuario','$id_cliente')");
        }
    } else {
    ...
    '$paterno', '$materno', '$nacimiento', '$email', '$calle', '$exterior', '$interior', '$estado', '$municipio', '$colonia', $postal, 2, 1)");
        ...
        $mensajes = 'El usuario: ' . $nombre_usuario . ' Activo la facturacion del cliente: ' . $numero_cliente;
        $log = mysqli_query($conexion, "INSERT INTO `log`(`accion_log`, `id_usuario`, `id_cliente`) VALUES ('$mensajes,'$id_usuario','$id_cliente')");
    }
}
?>
<meta http-equiv="refresh"
    content="1; url=../../admin/clientes/facturacion/datos_facturacion.php?id=<?php echo $id_cliente ?>">