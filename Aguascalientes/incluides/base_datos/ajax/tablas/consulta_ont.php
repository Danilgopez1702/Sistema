...
$consulta = mysqli_query($conexion, "SELECT * FROM `inventario` WHERE `tipo_inventario` = 2");

$data = array();

while ($datas = mysqli_fetch_assoc($consulta)) {
	$id_inventario = $datas['id_inventario'];
	$ont_inventario = $datas['ont_inventario'];
	$mac_ont_inventario = $datas['mac_ont_inventario'];
	...
	$data[] = [
		...
		'ont_inventario' => $ont_inventario,
		'mac_ont_inventario' => $mac_ont_inventario,
		'asignado' => $asignado,
