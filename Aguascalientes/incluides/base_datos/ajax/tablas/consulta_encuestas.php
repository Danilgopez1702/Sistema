...
if ($acomodo == 1) {
	$consulta = mysqli_query($conexion, "SELECT * FROM `encuesta` AS encuesta INNER JOIN cliente AS cliente ON encuesta.id_cliente = cliente.id_cliente");
} else if ($acomodo == 2) {
	$consulta = mysqli_query($conexion, "SELECT * FROM `encuesta` AS encuesta INNER JOIN cliente AS cliente ON encuesta.id_cliente = cliente.id_cliente  WHERE encuesta.status_encuesta = 2");
} 
...
while ($info = mysqli_fetch_assoc($consulta)) {
	...
	$fecha_instalacion = $info['fecha_instalacion'];
