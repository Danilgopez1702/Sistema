...
$consulta = mysqli_query($conexion, "SELECT * FROM `inventario` WHERE `tipo_inventario` = 1");

$data = array();

while ($datas = mysqli_fetch_assoc($consulta)) {
	$id_inventario = $datas['id_inventario'];
	$onu_inventario = $datas['onu_inventario'];
	$mac_inventario = $datas['mac_inventario'];
	...
	$data[] = [
		...
		'onu_inventario' => $onu_inventario,
		'mac_inventario' => $mac_inventario,
		'asignado' => $asignado,
