...
$sql_select = mysqli_query($conexion, "SELECT `mac_inventario`, `id_instalador`, `tipo_inventario` FROM `inventario` WHERE `mac_inventario`= '$nummac' AND `id_instalador` = 2 ");
$sql_select2 = mysqli_query($conexion, "SELECT `mac_inventario`, `id_instalador`, `tipo_inventario` FROM `inventario` WHERE `mac_inventario`= '$nummac' AND `id_instalador` != 2 ");
$numero = mysqli_num_rows($sql_select);
$numero2 = mysqli_num_rows($sql_select2);

if($numero >= 1){
    $sql_tecnico = mysqli_query($conexion,"SELECT * FROM `usuario` WHERE `id_usuario` = '$instalador_onu' ");
    ...
    echo "success";
    $mensaje = 'Se asigno la onu '. $nummac . ' al tecnico '.$nombre;
