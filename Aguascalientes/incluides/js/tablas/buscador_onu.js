$(document).ready(function () {
    ...
    var table = $('#dataTable').DataTable({
      "ajax": {
        "url": "../../../base_datos/ajax/tablas/consulta_onu.php",
  ...
  $(document).on("click", ".btnEditar", function () {
    ...
    onu_inventario = fila.find('td:eq(1)').text();
    mac_inventario = fila.find('td:eq(2)').text();
    id_instalador = fila.find('td:eq(3)').text();
    fallo_instalador = fila.find('td:eq(4)').text();
    $("#editar_id_inventario").val(id_inventario);
    $("#editar_mac_inventario").val(mac_inventario);
    $("#editar_onu_inventario").val(onu_inventario);
    $("#editar_id_instalador").val(id_instalador);
    $("#editar_fallo_inventario").val(fallo_instalador);
    $('#modal_editar_onu').modal('show');
  ...
  $(document).on("click", ".btnBorrar", function () {
    ...
    var y = confirm('Estás seguro que deseas eliminar la ONU');
    if (y) {
      window.location.href = "/digitalnetsistem/Aguascalientes/incluides/base_datos/eliminar/eliminar_inventario.php?id=" + user_id+"&&in=1";
    }
