...
<div class="card shadow mb-4">
    ...
    <div class="card-body ">
        <table class="table table-sm" id="dataTable" width="100%" cellspacing="0">  
            ...
            <tbody>
                    <?php
                        $query = mysqli_query($conexion, "SELECT * FROM `inventario` WHERE `tipo_inventario` = 1");
                        $result = mysqli_num_rows($query);
                        if ($result > 0) {
                            while ($data = mysqli_fetch_assoc($query)) { ?>
                                <tr>
                                    <td><?php echo $data['id_inventario'];?></td>
                                    <td><?php echo $data['onu_inventario'];?></td>
                                    <td><?php echo $data['mac_inventario'];?></td>
                                    <td>
                                        <?php 
                                    ...
                                    onclick='editar(
                                        ...
                                        "<?php echo $data["onu_inventario"];?>",
                                        "<?php echo $data["mac_inventario"];?>",
                                        "<?php echo $data["id_instalador"];?>",
                                        "<?php echo $data["fallo_inventario"];?>"
                                    )'><i class="fas fa-thin fa-pen ml-2"></i></a>
                                    <a title="Eliminar Onu" onclick="return confirm('Estás seguro que deseas eliminar la onu: <?php echo $data['onu_inventario'];?>');" href="../../../base_datos/eliminar/eliminar_onu.php?id=<?php echo $data['id_inventario'] ?>"><i class="fa fa-trash ml-2"></i></a>
                                    </td>
                                </tr>
                                <?php
                            }
...
<script src="../../../js/inventario/ver/onu.js"></script>
