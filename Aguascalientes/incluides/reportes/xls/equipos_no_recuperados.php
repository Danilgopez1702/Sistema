...
$filename = "inventar_clientes_equipos_no_recuperados_" . date("d-m-Y");         //File Name
...
$query = mysqli_query($conexion, "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'inventar_sistemadn' AND TABLE_NAME = 'cliente'");
...
$sql = mysqli_query($conexion, "SELECT `id_cliente`, `numero_cliente`, `folio_cliente`, `status_cliente`, `paquete_cliente`, `velocidad_cliente`, `precio_cliente`, 
`fecha_instalacion`, `fecha_corte`, `ip_cliente`, `vendedor_cliente`, `id_instalador`, `nombre_cliente`, `apellido_p_cliente`, `apellido_m_cliente`, `fecha_nacimiento`, 
`email_cliente`, `calle_cliente`, `numero_ext`, `numero_int`, `municipio`, `estado`, `colonia_cliente`, `codigo_postal`, `entre_calle1`, `entre_calle2`, `ref_dom`, 
`tel1_cliente`, `tel2_cliente`, `tel3_cliente`, `ref_nombre1`, `ref_tel1`, `ref_nombre2`, `ref_tel2`, `radio_cliente`, `router_cliente`, `onu_cliente`, `ont_cliente`, 
`bandera_cliente`, `bote_cliente`, `puerto_cliente`, `factura`, `por_revisar`, `id_zona`, `cron_checador`, `TIMESTAMP` FROM `cliente` WHERE `status_cliente` = 5");
...
if ($result_sql > 0) {
    while ($data_sql = mysqli_fetch_assoc($sql)) {
        ...
        $municipio_cliente = $data_sql['municipio'];
        $estado_cliente = $data_sql['estado'];
        ...
        $ont_cliente = $data_sql['ont_cliente'];
       ...
       echo $id_cliente . "\t" . $numero_cliente . "\t" . $folio_cliente . "\t" . $status_cliente  . "\t" . $paquete_cliente  . "\t" .  $velocidad_cliente . "\t" .
            ...
            . "\t" . $calle_cliente  . "\t" . $numero_ext  . "\t" . $numero_int ."\t" .  $municipio_cliente  ."\t" .  $estado_cliente  . "\t" .  $colonia_cliente  . "\t" . $codigo_postal . "\t" .  $entre_calle1
            . "\t" . $entre_calle2 . "\t" .  $ref_dom . "\t" . $tel1_cliente  . "\t" . $tel2_cliente  . "\t" . $tel3_cliente  . "\t" . $ref_nombre1
            . "\t" . $ref_tel1 . "\t" .  $ref_nombre2  . "\t" . $ref_tel2 . "\t" .  $radio_cliente  . "\t". "\t" .  $router_cliente
