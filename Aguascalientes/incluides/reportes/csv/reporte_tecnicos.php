...
$tecnico = $_POST['tecnicos_reparacion'];
...
if ($tecnico == 9999) {
    $query_reparaciones = mysqli_query($conexion, "SELECT quejas.reparador, clientes.nombre, clientes.apellido_paterno, clientes.apellido_materno, 
    clientes.num_cliente, clientes.calle, clientes.numero_ext, clientes.numero_int, clientes.colonia, clientes.ref_dom, clientes.tel1, clientes.tel2, 
    clientes.onu, clientes.ont, clientes.num_equipo, quejas.fechaAsignacion, quejas.noReporte, quejas.mensaje, quejas.porRevisar, usuarios.usuario 
    FROM `quejas` LEFT JOIN clientes ON quejas.idClientes = clientes.idClientes LEFT JOIN usuarios ON quejas.idUsuarios = usuarios.idUsuarios 
    WHERE quejas.porRevisar != 0 AND quejas.tipo = 'Reparaciones' AND quejas.noReporte != '' AND quejas.reparador != ''");

    $query_reparaciones = mysqli_query($conexion, "SELECT reportes.id_usuario, cliente.nombre_cliente, cliente.apellido_p_cliente, cliente.apellido_m_cliente, 
    cliente.numero_cliente, cliente.calle_cliente, cliente.numero_ext, cliente.numero_int, cliente.colonia_cliente, cliente.estado, cliente.municipio, cliente.ref_dom, 
    cliente.tel1_cliente, cliente.tel2_cliente, cliente.onu_cliente, cliente.ont_cliente, cliente.radio_cliente, reportes.fecha_creacion, reportes.no_reporte_reportes, 
    reportes.mensaje_reportes, reportes.status_reportes, reportes.id_usuario FROM reportes LEFT JOIN cliente ON reportes.id_cliente = cliente.id_cliente LEFT JOIN usuario 
    ON reportes.id_usuario = reportes.id_usuario WHERE reportes.status_reportes");

    $schema_insert = "nombre,fecha reporte,estatus,# reporte,numero de cliente,mensaje,dio de alta,calle,# exterior,# interior,colonia,referencia,telefono1,
    telefono2,onu,ont,antena,reparador";
